    ...
    <>
      {openChat ? (
        <>
          ...
          />
          <div className="fixed bottom-14 z-[10002] right-14 rounded-xl overflow-hidden">
            <div className="shadow-md flex flex-col rounded-xl border-gray-200 w-[400px] h-[70vh] overflow-hidden">
              ...
              <div
                className="bg-white flex-1 h-full overflow-auto flex flex-col gap-4 pt-4 pb-[80px] px-4"
                ref={lastMessageRef}
              >
        ...
        <button
          onClick={openChatRoom}
          className="fixed bottom-10 right-10 z-[10000] bg-primary h-12 w-12 flex items-center justify-center rounded-full"
        >
          <ChatDotIcon className="w-8 h-8 text-white" />
          {unReadingCount ? (
            <span className="text-white bg-red-500 h-7 w-7 absolute -top-2 -right-1 rounded-full">
              {unReadingCount}
            </span>
          ) : null}
        </button>
